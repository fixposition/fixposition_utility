...
// See also https://reveng.sourceforge.io/crc-catalogue/all.htm, https://users.ece.cmu.edu/~koopman/crc/,
// https://en.wikipedia.org/wiki/Cyclic_redundancy_check

// ---------------------------------------------------------------------------------------------------------------------

// clang-format off
// CRC table generated using https://github.com/madler/crcany.git, https://reveng.sourceforge.io/crc-catalogue/all.htm
// width=32 poly=0x32c00699 init=0x00000000 refin=false refout=false xorout=0x00000000 check=0x00000000 residue=0x000000 name="FF-FPB-32"
// The chosen polynomial is this one from http://users.ece.cmu.edu/~koopman/crc/crc32.html:
//    (0x9960034c; 0x132c00699) <=> (0x9960034c; 0x132c00699) {65506,65506,32738,32738,193,193,31,31,17,17} | gold | (*o) CRC-32K/6.4
// For HD = 5 and 6: 32738 data word length [bits], which is ~ 4 KiB, which is suitable for the expected FP_B maximum message size (of 1-2 KiB typically)
// clang-format on
static uint32_t const k_crc32_fpb[] =  // clang-format off
{
...
static const uint32_t k_crc24_rtcm3[256] =  // clang-format off
{
    // width=24 poly=0x864cfb init=0x000000 refin=false refout=false xorout=0x000000 check=0x000000 residue=0x000000 name="RTCM3_SPARTN24"
...
uint32_t Crc24rtcm3(const uint8_t* data, const std::size_t size)
{
    ...
    if (data != NULL) {
        for (std::size_t ix = 0; ix < size; ix++) {
            crc = (crc << 8) ^ k_crc24_rtcm3[((crc >> 16) ^ data[ix]) & 0xff];
        }
    }
...
static const uint32_t k_crc32_novb[256] =  // clang-format off
{
    // width=32 poly=0x04c11db7 init=0x00000000 refin=true refout=false xorout=0x00000000 check=0x00000000 residue=0x000000 name="NOV_B-32"
...
static const uint8_t k_crc4_spartn[256] =  // clang-format off
{
    // width=4 poly=0x9 init=0x0 refin=true refout=true xorout=0x0 check=0x0 residue=0x0 name="SPARTN-4"
...
static const uint8_t k_crc8_spartn[256] =  // clang-format off
{
    // width=8 poly=0x07 init=0x00 refin=false refout=false xorout=0x0 check=0x0 residue=0x0 name="SPARTN-8"
...
static const uint16_t k_crc16_spartn[256] =  // clang-format off
{
    // width=16 poly=0x1021 init=0x0000 refin=false refout=false xorout=0x0000 check=0x0000 residue=0x0000 name="SPARTN-16"
...
uint16_t Crc16spartn(const uint8_t* data, const std::size_t size)
{
    ...
    if (data != NULL) {
        for (std::size_t ix = 0; ix < size; ix++) {
            crc = (crc << 8) ^ k_crc16_spartn[((crc >> 8) ^ data[ix]) & 0xff];
        }
    }
    return crc;
}
...
static const uint32_t k_crc32_spartn[256] =  // clang-format off
{
...
uint32_t Crc32spartn(const uint8_t* data, const std::size_t size)
{
    ...
    if (data != NULL) {
        for (std::size_t ix = 0; ix < size; ix++) {
            crc = (crc << 8) ^ k_crc32_spartn[((crc >> 24) ^ data[ix]) & 0xff];
        }
    }
    return crc;
}
